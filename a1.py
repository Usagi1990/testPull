...
def bfs(graph, root, max_depth):
    ...
    q.append(root)
    seen_list.append(root)
    node2num_paths[root] = 1

    while len(q) > 0:
        node = q.popleft()
        for n_neighbor in graph.neighbors(node):
            if n_neighbor not in seen_list:
                seen_list.append(n_neighbor)
                node2distances[n_neighbor] = node2distances[node] + 1
                node2num_paths[n_neighbor] = node2num_paths[node]
                node2parents[n_neighbor].append(node)
                if node2distances[n_neighbor] < max_depth:
                    q.append(n_neighbor)
            else:
                if node2distances[n_neighbor] == node2distances[node] + 1:
                    node2num_paths[n_neighbor] += node2num_paths[node]
                    node2parents[n_neighbor].append(node)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    """
    node2distances = sorted(node2distances.items(), key=lambda d: d[1], reverse=True)
    node_score ={}
    result = {}
    for node, value in node2distances:
        if node not in node_score: node_score[node] = 1
        for aimnode in node2parents[node]:
            if aimnode not in node_score: node_score[aimnode] = 1
            node_score[aimnode] = float(node_score[aimnode] + node_score[node] * node2num_paths[aimnode] / node2num_paths[node])
            key = tuple(sorted((node, aimnode)))
            result[key] = node_score[node] * node2num_paths[aimnode] / node2num_paths[node]
    return result
...
def partition_girvan_newman(graph, max_depth):
    ...
    #pass
    copied = graph.copy()
    betweenness_result = approximate_betweenness(graph, max_depth)
    betweenness_result = sorted(betweenness_result.items(), key=lambda d: (-d[1], d[0][0], d[0][1]))
    for edge in betweenness_result:
        copied.remove_edge(*edge[0])
        if len(list(nx.connected_component_subgraphs(copied))) > 1: break
    return list(nx.connected_component_subgraphs(copied))
...
def get_subgraph(graph, min_degree):
    ...
    ###TODO
    #pass
    removed_node_list = [n for n in graph.nodes() if graph.degree(n) < min_degree]
    graph.remove_nodes_from(removed_node_list)
    return graph
...
def volume(nodes, graph):
    ...
    copied = graph.copy()
    for node in nodes:
      cnt += sum(1 for i in copied.neighbors(node))
...
def cut(S, T, graph):
    ...
    # result = 0
    # for startnode in S:
    #   if endnode in graph.neighbors(startnode):
    #     result += (endnode in T) &1
    # return result
...
def norm_cut(S, T, graph):
    ...
    # cut = cut(S, T, graph)
    # return cut/volume_S + cut/volumn_T
...
def brute_force_norm_cut(graph, max_size):
    ...
    for i in range(max_size):
         for combo in list(combinations(edges, i+1)):
                copied = graph.copy()
                for edge in combo:
                    copied.remove_edge(*edge)
                if len(get_components(copied)) == 2: 
                    components = get_components(copied)
                    result.append((norm_cut(components[0], components[1], graph), combo))
    return result
...
def score_max_depths(graph, max_depths):
    ...
    for max_depth in max_depths:
      ...
      result.append((max_depth, norm_cut(components[0], components[1], graph)))
    return result
...
def make_training_graph(graph, test_node, n):
    ...
    removeList = []
    copied = graph.copy()
    test_neighbors_list = sorted(graph.neighbors(test_node))
    for node in test_neighbors_list:
      if n ==len(removeList): break
      removeList.append((test_node,node))
    copied.remove_edges_from(removeList)
    return copied
...
def jaccard(graph, node, k):
    ...
    node_try = [x for x in graph.nodes() if x != node and x not in graph.neighbors(node)]
    for neighbors1 in node_try:
        neighbors2 = set(graph.neighbors(neighbors1))
        scores_result.append(((node, neighbors1), (1. * len(neighbors & neighbors2) / len(neighbors | neighbors2))))
    scores_result = sorted(scores_result, key=lambda tuple: (-tuple[1], tuple[0][1]))
    return scores_result[:k]
...
def evaluate(predicted_edges, graph):
    ...
    for edge in predicted_edges:
      if graph.has_edge(*edge):
        cnt +=1
    return cnt/len(predicted_edges)
